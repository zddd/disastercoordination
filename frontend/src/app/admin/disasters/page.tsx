...
export default function DisastersPage() {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/api/v1/disasters")
      .then((r) => r.json())
  ...
  const handleCreate = async () => {
    await fetch("http://localhost:8080/api/v1/disasters", {
      method: "POST",
  ...
  const handleClose = async (id: string) => {
    if (!confirm("确认关闭此灾害？此操作不可撤销。")) return;
    await fetch(`http://localhost:8080/api/v1/disasters/${id}/close`, { method: "PUT" });
    window.location.reload();
  };
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="p-3 text-left">类型/等级</th>
            ...
            {disasters.map((d) => (
              <tr key={d.id} className="border-t">
                ...
                <td className="p-3 text-gray-500">
                  {d.type} · <span className={`text-xs px-1 rounded ${
                    d.level === "red" ? "bg-red-100 text-red-700" :
                    d.level === "orange" ? "bg-orange-100 text-orange-700" :
